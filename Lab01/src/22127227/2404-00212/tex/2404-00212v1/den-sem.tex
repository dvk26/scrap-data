...
\begin{enumerate}
  \item{\emph{Computational}: \(\mathbb{C}\) is a \(\Sigma\)-decidable predomain. }  
  \item{\emph{Phase separation}: \(\mathbb{C}\) is purely intensional, \ie $(\P \to \mathbb{C}) \cong 1$. }  
  \item{\emph{Discretely ordered}: \(\mathbb{C}\) is discrete in the sense of \cref{def:discrete}. }
\end{enumerate}

We require $\mathbb{C}$ to be purely intensional to ensure that the denotational semantics of \pcfc{} exhibits an \emph{intrinsic} noninterference property of cost and behavior as sketched in \cref{subsec:den-sem}. As discussed in \cref{subsec:predomains}, there is a canonical way of turning any monoid $\mathcal{M}$ into a purely intensional type by means of the sealing monad $\P \vee -$. The remaining two conditions have to do with the technical requirements of the computational adequacy proof: when reasoning about the \emph{computational semantics} of \pcfc{} (see \cref{sec:comp-sem}), we need $\mathbb{C}$ to be discrete in order to prove the property that sequential composition of computations may be decomposed (\cref{prop:comp-ind-seq}), and we need $\mathbb{C}$ to be $\Sigma$-decidable when showing that the \emph{formal approximation predicates} (see \cref{fig:formal-approx-rel}) associated to semantic domains are admissible. 
...
To model the cost effect $\kw{step} : \mathbb{C} \times X \to X$, we use the fact that every algebra for a composite monad is canonically an algebra for the constituent monad. In our case this means that every $\mathbb{T}$-algebra has an underlying cost algebra as well; we write $\costmap : \mathbb{C} \times X \to X$ for the cost algebra map. 
