...
\begin{definition}
  Given a type $A$, the \emph{lift} of $A$ is defined as $\lift{A} = \Sigma_{\phi : \Sigma}.~\phi \to A$. 
\end{definition}
...
\begin{nota}
  Given a partial element \(e :  \mathsf{L} A\), we write \(e { \downarrow }  :  \Sigma\) for its support. When it is known that \(e { \downarrow }\) holds, we may write \(e : A\) for the defined element. 
\end{nota}
...
Complete types are the synthetic counterpart to $\omega$-cpos in classic domain theory. The class of well complete types is the least restrictive possible notion of predomain is closed under lifting. In this paper we consider the dual \emph{most restrictive} class of predomain, the \emph{replete} types, in order to obtain a sharper characterisation of the intrinsic order relation (see \cref{subsec:intrinsic}) on predomains. 
...
\begin{definition}\label[defn]{def:axioms}
  An \emph{SDT model of the intension-extension phase distinction} is an elementary topos \CatIdent{E} equipped with a dominance $\Sigma$ satisfying Phoa's principle such that $\Sigma$ is complete, and a distinguished $\Sigma$-proposition $\P$ such that the type of booleans $2$ is a $(\P\to -)$-modal predomain. 
\end{definition}
