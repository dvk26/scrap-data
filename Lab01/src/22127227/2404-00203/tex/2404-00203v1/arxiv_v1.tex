...
\title{\myfont{A Stackelberg Regret Minimizing Framework for Online Learning in Newsvendor Pricing Games}}
...
Our supply chain game consists of a leader (supplier) and a follower (retailer). The leader dynamically prices the item to sell to the follower, and the follower must thereafter dynamically price the item for the market. Differing from \citet{cesa:2022-supp-chain-games}, in addition to determining the optimal order amount (also known as the \textit{Newsvendor} problem), the follower must also as dynamically price the product in the face of an demand uncertainty (known as the \textit{price-setting Newsvendor}). Thus we introduce the problem of demand learning coupled with inventory risk. The objective of this repeated game is to determine the optimal policy to maximize the expected profit in the face of stochastic demand for the leader, and determine the best response of the follower (under the assumptions in Section \ref{sec:assumptions}), leading to an approximate Stackelberg equilibrium. The rationale behind this is that, often times, decisions between firms competing with one another do not occur simultaneously, but rather turn-based. We define this \textit{Newsvendor pricing game} more concretely in Section \ref{sec:newsvendor-games}. 
...
\textbf{Regret Definition:} The cumulative value of regret, defined as $R_A^T(\cdot)$ for the leader, and $R_B^T(\cdot)$ for the follower, is the difference in cumulative rewards when players optimize their strategies in hindsight versus using the joint strategy $(\pi_A, \pi_B)$. In hindsight, both players' reward functions are known, and each player is seeking to maximize their rewards rationally. As the state does not transition in the bandit problem, given the leader's strategy $\pi_A$ is a sequence of actions $\mathbf{a}^t$, $\forall t \in \{1, \dots, T \}$, the optimal follower strategy, $\pi_B$, is constituted by the optimal reaction to $\mathbf{a}^t$, denoted as $\mathfrak{B}(\mathbf{a}^t)$. 
...
\begin{align}
    ...
    R_B^T(\pi_A, \pi_B) &= \mathbbm{E}_\theta \Big[ \sum^T_{t = 1} \mathcal{G}_B(\mathbf{a}^t, \mathfrak{B}(\mathbf{a}^t)) - \sum^T_{t = 1} \mathcal{G}_B(\mathbf{a}^t, \pi_B(\mathbf{a}^t))\Big]  \label{eq:follower-regret} %\label{eq:follower-regret}
    % \tilde{\mathfrak{B}}(\mathbf{a}^t) = \big\{ \mathbf{b} : \underset{\mathbf{b} \in \mathcal{B}}{\mathrm{argmax}} \ \mathbbm{E}[\mathcal{G}_B(\mathbf{a}^t, \mathbf{b})] \big\} \label{eq:br-approx-def}
\end{align}

Follower regret, defined as $R_B^T(\cdot)$ in Eq. \eqref{eq:follower-regret}, is the difference between cumulative follower rewards from the best-responding follower and any other strategy adhered to by the follower in response to a series of actions form the leader, $\{ \mathbf{a}^1, \dots, \mathbf{a}^T \}$. In this problem setting, when provided with a strategy $\pi_A$, the follower does not attempt to strategically alter the future behavior of the agent, rather he aims to maximize his current rewards at time $t$. From this perspective, the follower's regret reduces to a stochastic contextual bandit setting. Given context $\mathbf{a}^t$ and environmental parameters $\theta$, the follower attempts to maximize his rewards via some no-regret learner (we choose an optimistic stochastic linear contextual bandit algorithm as outlined in Section \ref{sec:learning-algorithms}). Regarding the value of expectations, $\mathbbm{E}_{\pi_B}[\mathcal{G}_A(\cdot)]$ indicates that the uncertainty is with respect to the strategy of the follower, and $\mathbbm{E}_{\theta}[\mathcal{G}_B(\cdot)]$ indicates that uncertainty is with respect to the stochastic environment, governed by parameter $\theta$. For convenience of notation, moving forward, we will establish the equivalences for $\mathbbm{E}_{\pi_B}[\mathcal{G}_A(\cdot)] \equiv \mathbbm{E}[\mathcal{G}_A(\cdot)]$ and $\mathbbm{E}_\theta[\mathcal{G}_B(\cdot)] \equiv \mathbbm{E}[\mathcal{G}_B(\cdot)]$.

\textit{Stackelberg regret}, also known as the \textit{external regret} \cite{haghtalab:2022-stack-non-myo} \cite{zhao:2023-online} is defined from the leader's perspective in Eq. \eqref{eq:leader-regret-def} as the cumulative difference in leader's rewards between the maximization of $\mathcal{G}_A(\cdot)$ under a best responding follower, versus any other joint strategy adhered to by both agents. This definition assumes that the rational leader must commit to a strategy $\pi_A$ at the beginning of the game, which she believes is optimal on expectation. The leader maximizes their reward taking into account their prediction of the follower's response $\mathbf{a}^t$, which can be suboptimal compared to a maximization of $\mathcal{G}_A(\mathbf{a}, \mathfrak{B}(\mathbf{a}^t))$ over $\mathbf{a} \in \mathcal{A}$. When computing the expectation of $\mathcal{G}_A(\cdot)$, the randomness of $\mathcal{G}_A(\cdot)$ is fully strategic, that is arising from the strategy of $\pi_B$. Our algorithm seeks to minimize the Stackelberg regret, and constitutes a no-regret learner from the leader's perspective.


\subsection{Approximate Stackelberg Equilibrium} \label{sec:stack-eq-defn}


In our problem setting of economic competition between firms in a non-cooperative supply chain, our  objective is to learn an approximate Stackelberg equilibrium (SE), through the approximation of the best response function $\mathfrak{B}(\cdot)$. Acknowledging the presence of the approximation error of best response function, we derive an \textit{$\epsilon$-approximate equilibrium} with full support, \cite{roughgarden:2010-algorithmic} \cite{daskalakis:2009complexity}. To note, the follower has an opportunity to observe and respond to the action of the leader, and his strategy is therefore defined as $\pi_B(\mathbf{a})$. To formally define an approximate two-player \textit{$\epsilon$-equilibrium}, we stipulate that there exists some $\epsilon_A \geq 0$ such that, when the leader adopts a pure $\epsilon$-equilibrium strategy $\hat{\mathbf{a}}$, we obtain the following inequality,
...
Where $\Delta_B$ denotes a probability simplex over possible strategies $\pi_B$, and thus the expectation $\mathbbm{E}_{\pi_B}[\cdot]$ is with respect to $\pi_B$. From the follower's perspective, suppose he adopts an approximate best response strategy $\hat{\pi}_B \equiv \hat{\pi}_B(\mathbf{a})$. This implies that there exists some $\epsilon_B \geq 0$, such that,

\begin{align}
     \mathbbm{E}_\theta [ \mathcal{G}_B(\mathbf{a}, \hat{\pi}_B) ] \geq \mathbbm{E}_\theta [\mathcal{G}_B(\mathbf{a}, \mathfrak{B}(\mathbf{a}))] - \epsilon_B, \quad \forall \mathbf{a} \in \mathcal{A} 
\end{align}

This formalization allows for an $\epsilon$ deviation, making the problem computationally tractable, particularly when optimization over certain functions lacks simple closed-form solutions or is computationally intensive (as we explore in detail in Section \ref{sec:econ-newsv-theory}). It's worth noting that the expectation for the leader reflects uncertainty about the follower's strategy, while the expectation for the follower relates to the randomness of the environment, denoted by parameter $\theta$. 


% our approximate result corresponds to a  \textit{correlated equilibrium} \cite{aumann:1987correlated} for the follower, and a \textit{coarse correlated equilibrium} \cite{roughgarden:2010-algorithmic} for the leader. From an intuitive perspective, the best response of the follower is restricted by a range for any given action $\mathbf{a}$. A response under perfect information, the leader is unable to act in response to a the follower, and must commit to a pure strategy before hand. 


\textbf{Upper-Bounding Approximation Function:} Let $\mathcal{U}_B(\mathbf{a}, \mathbf{b})$ represent some function, $\mathcal{U}_B : \mathbf{a} \times \mathbf{b} \to \mathbb{R}^+$ such that it is consistently greater than or equal to $\mathbbm{E}[\mathcal{G}_B(\mathbf{a}, \mathbf{b})]$.
...
We define the approximate follower best response as,
...
Effectively, $\epsilon_B$ defines an upper bound of the range on where $\mathop{\mathbb{E}} [\mathcal{G}_B(\mathbf{a}, \mathfrak{B}(\mathbf{a}))]$ could lie, given the approximate best response under perfect information, defined as $\widetilde{\mathfrak{B}}(\mathbf{a})$. $\epsilon_B$ represents the worst case approximation error of $\mathbbm{E}[\mathcal{G}_B(\mathbf{a}, \mathfrak{B}(\mathbf{a}))]$, where $\epsilon_B > 0$. 
...
\begin{table*}\centering 
    ...
    \begin{tabular}{p{1.0in}p{1.0in}p{0.35in}p{1.1in}p{0.6in}p{1.2in}} \toprule
...
\subsection{Newsvendor Pricing Game (NPG)} \label{sec:newsvendor-games}

We model the two learning agents in a \textit{Newsvendor pricing game}, involving a supplier $A$ and a retailer $B$. The leader, a supplier, is learning to dynamically price the product for the follower, a retailer, aiming to maximize her reward. The follower, a \textit{price-setting Newsvendor}, must determine both a market resale price and an order quantity. To achieve this, the follower adheres to classical Newsvendor theory, which involves finding the optimal order quantity given a known demand distribution before the realization of the demand. This well-known problem involves the computation of a critical fractile, a key parameter in classical Newsvendor models used to determine the optimal order amount for perishable goods \cite{arrow:1951newsboy}. Additionally, the retailer must dynamically price the product for resale to the market, with his goal being the determination of the optimal resale price that yields the highest expected profits. We provide further economic details in Section \ref{sec:econ-newsv-theory}.

\subsubsection{Rules of the Newsvendor Pricing Game} \label{sec:rules_of_npg}

Moving forward away from abstraction, and for simplicity, we denote $a \equiv \mathbf{a} \in \mathbbm{R}^1$, and $\mathbf{b} \equiv [b, p]^\intercal \in \mathbbm{R}^2$. Where $a$ denotes wholesale price from the supplier firm, $p$ and $b$ denote the retail price and order amount of the retail firm.
...
\begin{enumerate} 
    ...
    \item The follower is myopic, it maximizes for its immediate reward under the available information.
...
\section{Learning Algorithms} \label{sec:learning-algorithms}
...
The upper bound of the optimistic best response, $\bar{b}_a$ (Eq. \eqref{eq:ba_upper_bound_true}), can be determined by estimating the optimistic riskless price $\bar{p}_0$. Let $\mathbb{E}[\mathcal{G}_B(\theta|a)]$ denote the expected profit under parameter $\theta$, give $a$. Both the left and right terms of Eq. \eqref{eq:max-theta} are optimistic estimates of expected profit, $\mathbb{E}[\mathcal{G}_B(\theta|a)]$, using the riskless price $p_0$ as a surrogate for the optimal price. Under a myopic strategy (as outlined in Section \ref{sec:assumptions}), the follower's pricing with $\bar{p}_0$, and ordering with $\bar{b}_a$, under an optimistic estimate of the demand parameters $\theta$ given data $\mathcal{D}^t$, can be summarized from Eq. \eqref{eq:max-theta} to Eq. \eqref{eq:ba_upper_bound_true}. (The derivation from Eq. \eqref{eq:max-theta} to Eq. \eqref{eq:ba_upper_bound_true} is in Appendix \ref{sec:notes-opt-br}.) 
...
\textbf{Sketch of Proof:} Given the regret properties of the leader in Theorem \ref{thm:leader-regret} and follower in Theorem \ref{thm:follower-regret}, as the asymptotic performance of instantaneous regret approaches 0 as $t \xrightarrow[]{} \infty$, then the algorithm achieves a Stackelberg equilibrium. We have a myopic follower who will always be suboptimal in terms of instantaneous reward, due to always pricing at the riskless price $p_0$ instead of $p^*$. 
...
% LinUCB utilizes linear regression models to estimate the upper confidence bound on reward distributions associated in a continuous action space. We use this as a baseline measure when computing empirical follower regret. 
...
\endminipage\hfill
\caption{Episodic reward and Stackelberg (leader) regret performance across an Newsvedor pricing game, comparing Algorithm \ref{alg:se-newsv} (LNPG) against UCB. (Parameters used $\kappa=3, \theta_0 = 18, \theta_1 = 7$.)  } \label{fig:stack_regret}
\end{figure}
...
\begin{proof} 
    We assume $\bar{V}_t$ is full rank, and that sufficient sufficient exploration has occurred. Let $\mathbf{\Delta}_\theta = \theta^* - \hat{\theta}_t$. From \citet{abbasi:2011ofu}, we define $\bar{V}_t$, 
    ...
    We note that $||\theta - \theta^*||_{\bar{V}_t} \leq \kappa \sqrt{\log(t)}$ with non-decreasing eivengalues of $\bar{V}_t$ with respect to $t$, this generally denotes a volume decreasing ellipsoid \cite{lattimore:2020-bandit}. Suppose there exists a scalar multiple $\kappa_2$, where is exists in the range $\kappa_2 \in (0, \infty)$. Given the finite time behaviour $t \leq T$, there must exist some $\gamma$ such that the following bound holds,

    \begin{align}
        \kappa_2 \sqrt{t} ||\mathbf{\Delta}_\theta||_2 \leq \sqrt{\mathbf{\Delta}_\theta^T \bar{V}_t  \mathbf{\Delta}_\theta} = \norm{\mathbf{\Delta}_\theta}_{\bar{V}_t} \label{eq:eta_ineq}
    \end{align}

    In essence, as $\kappa_2 \in (0, \infty)$, we can always choose $\kappa_2$ to be small enough such that the inequality the inequality in Eq. \eqref{eq:eta_ineq} holds in finite time, as $||\mathbf{\Delta}_\theta||_2$ is constant and $||\mathbf{\Delta}_\theta||_{\bar{V}_t }$ is non-decreasing with respect to time. Therefore, the existence of a scalar constant $\kappa_2$ always exists, such that,
...
\minipage{0.5\textwidth}
  ...
  % \caption*{Empirical follower regret. $\kappa=4, \theta_0 = 10, \theta_1 = 3$.}%\label{fig:market1_loss}
\endminipage\hfill
...
\minipage{0.5\textwidth}
  ...
  %\caption*{Empirical follower regret. $\kappa=2, \theta_0 = 10, \theta_1 = 3$.}%\label{fig:market1_loss}
\endminipage\hfill
\caption*{Parameters: $\kappa=0.05, \theta_0 = 40, \theta_1 = 5, \sigma=5.8$.}
\caption{Regret performance of various experiments. Mean values are computed over 20,000 trials. All confidence intervals (shaded area) are within one standard deviation. } \label{fig:loss_plot}
\end{figure}
