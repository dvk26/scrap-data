...
\def\epen       {\ensuremath{e^+e^-}\xspace}
...
\newcommand{\mbc}{\ensuremath{M_{\rm bc}}\xspace}
...
% Some variables aliases.
\def\plab {\ensuremath{p_{lab}}\xspace}
\def\pt {\ensuremath{p_{T}}\xspace}
\def\thetalab {\ensuremath{\theta_{lab}}\xspace}
\def\thetaclus {\ensuremath{\theta_{cluster}}\xspace}
\def\eclus {\ensuremath{E_{cluster}}\xspace}
\def\eop {\ensuremath{E/p}\xspace}

% Ks -> pipi
\def\kspipi         {\ensuremath{K_{\rm S}^0\to\pi^+\pi^-}\xspace}
% Jpsi -> mumu
\def\jpsimumu       {\ensuremath{\jpsi \to \mup\mun}\xspace}
% Jpsi -> ee
\def\jpsiee         {\ensuremath{\jpsi \to \ep\en}\xspace}
% Jpsi -> ll
\def\jpsill         {\ensuremath{\jpsi \to \ell^+\ell^-}\xspace}
% ee -> ee (gamma)
\def\eegamma        {\ensuremath{\ep\en \to \ep\en (\gamma)}\xspace}
% ee -> mumu gamma
\def\mumugamma      {\ensuremath{\ep\en \to \mup\mun\gamma}\xspace}
% ee -> ll (gamma)
\def\llgamma        {\ensuremath{\ep\en \to \ell^+\ell^-(\gamma)}\xspace}
%% ee -> (ee) ee
\def\eeee           {\ensuremath{\ep\en \to (\ep\en)\ep\en}\xspace}
%% ee -> (ee) mumu
\def\eemumu         {\ensuremath{\ep\en \to (\ep\en)\mup\mun}\xspace}
%% ee -> (ee) ll
\def\eell           {\ensuremath{\ep\en \to (\ep\en)\ell^+\ell^-}\xspace}
%% ee -> (ee) pipi
\def\eepipi         {\ensuremath{\ep\en \to (\ep\en)\pi^+\pi^-}\xspace}
% D* -> D0(K pi) pi
\def\dstardkpipi    {\ensuremath{D^{*+}\to D^0(\to K^-\pi^+)\pi^+}\xspace}
% ee -> tau(1p) tau (3p)
\def\taupair        {\ensuremath{e^+e^-\to \tau^{\pm}(1P)\tau^{\mp}(3P)}\xspace}


\def\babar{\mbox{\sl B\hspace{-0.4em} {\small\sl A}\hspace{-0.37em} \sl B\hspace{-0.4em} {\small\sl A\hspace{-0.02em}R}}}
\def\btosll{\ensuremath{b\to s \ell^+ \ell^-}\xspace}
\def\BtoKstll{\ensuremath{B\to K^*\ell^+ \ell^-} \xspace}
\def\BtoKstee{\ensuremath{B\to K^*e^+ e^-} \xspace}
\def\BztoKstee{\ensuremath{B^0 \to K^{*0}e^+ e^-} \xspace}
\def\BptoKstee{\ensuremath{B^+ \to K^{*+}e^+ e^-} \xspace}
\def\BtoKstmumu{\ensuremath{B\to K^*\mu^+ \mu^-} \xspace}
\def\BztoKstmumu{\ensuremath{B^0 \to K^{*0}\mu^+ \mu^-} \xspace}
\def\BtoKstgamma{\ensuremath{B\to K^* \gamma} \xspace}
\def\BztoKstJpsi{\ensuremath{B^0 \to K^{*0}\Psi_{1S,2S}(\to e^+ e^-)} \xspace}
\def\BtoKsteta{\ensuremath{B\to K^* \eta} \xspace}
\def\BtoDstlnu{\ensuremath{B \to D^{*}\ell^+ \nu_\ell} \xspace}
\def\thetal{\ensuremath{\theta_{\ell}}}
\def\thetaK{\ensuremath{\theta_{K}}}
\def\ctl{\ensuremath{\cos{\thetal}}}
\def\ctk{\ensuremath{\cos{\thetaK}}}
\def\ATt{\ensuremath{A_T^{(2)}} \xspace}
\def\ATR{\ensuremath{A_T^{\text{Re}}} \xspace} 
\def\ATI{\ensuremath{A_T^{\text{Im}}} \xspace}
\def\tphi{\ensuremath{\tilde{\phi}}}
\def\pfp{\ensuremath{P_5^\prime} \xspace}

\newcommand{\gevccsq}{\ensuremath{{\mathrm{\,Ge\kern -0.1em V^2\!/}c^4}}\xspace}
\def\csev{\ensuremath{\mathcal{C}_7} \xspace}

\def\eid {\texttt{eID}\xspace}
\def\muid {\texttt{muID}\xspace}