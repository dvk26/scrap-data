...
\begin{align*}
    V_{t}\left(\pi_{t:T}\right) \equiv E\left[\sum_{s=t}^{T}\sum_{\underline{a}_{t:s} \in \underline{\MA}_{t:s}}\left( Y_{s}(\underline{A}_{(t-1)},\underline{a}_{t:s})\cdot \prod_{\ell=t}^{s}1\{\pi_{\ell}\left(H_{\ell}(\underline{a}_{(\ell-1)})\right)=a_\ell\}\right)\right],
\end{align*}
where we denote $Y_{s}(\underline{A}_{(0)},\underline{a}_{1:s}) = Y_{s}(\underline{a}_{1:s})$ for $t=1$. $V_{t}\left(\pi_{t:T}\right)$
